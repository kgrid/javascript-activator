...
export function resolveRelativeURL(baseURL: string, relativeFilename: string): string {
...
export async function download(source: string, destination: string): Promise<void> {
   ...
   * Unzip the file
   */
export async function unzip(filepath: string, destinationDir: string): Promise<void> {
    // We execute the command
    // The function returns details about the spawned process
    const process = Deno.run({
      cmd: ["unzip", "-q", filepath, "-d", destinationDir],
      stdout: "piped",
      stderr: "piped",
    });
