...
import { path, isURL, existsSync  } from "./deps.ts";
import {getFilenameFromURL, isLocalPathOrURL, download, unzip, resolveRelativeURL} from "./load.ts"
...
async function loadKO(itemLocation: string){
  ...
  if(!path.isAbsolute(itemLocation) && !isURL(itemLocation)){ //if itemLocation is relative, need to resolve towards manifest path
    if(isLocalPathOrURL(manifest_path)) itemLocation=resolveRelativeURL(path.toFileUrl(manifest_path).href,itemLocation); //make manifest path a uri so that it could be resolved
  ...
  try {
    ...
    const cacheFolder=collection_path + "/" + getFilenameFromURL(itemLocation).replace(path.extname(itemLocation),"");
    if(existsSync(localLocation) && !existsSync(cacheFolder)){
      await unzip(localLocation,collection_path);
    }
  } catch (error) {
    console.error("Error loading ko at ",itemLocation, ". error: ", error.message);
  }
}

async function main() {
  collection_path=Deno.env.get("ORG_KGRID_JAVASCRIPT_ACTIVATOR_COLLECTION_PATH") ?? "";
  if (collection_path == "" || undefined) {
    collection_path="./pyshelf"
  }

  manifest_path=Deno.env.get("ORG_KGRID_JAVASCRIPT_ACTIVATOR_MANIFEST_PATH") ?? "";
  if (manifest_path == "" || undefined) {
    console.log(`ORG_KGRID_JAVASCRIPT_ACTIVATOR_MANIFEST_PATH is not defined.`);
  } else {
    if(!path.isAbsolute(manifest_path) && isLocalPathOrURL(manifest_path)) manifest_path=path.resolve(manifest_path); //used to support relative manifest path
